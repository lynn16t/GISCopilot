...
QWebView = None
for _imp in (
    "from qgis.PyQt.QtWebEngineWidgets import QWebEngineView as _W",
    "from PyQt5.QtWebEngineWidgets import QWebEngineView as _W",
    "from PyQt6.QtWebEngineWidgets import QWebEngineView as _W",
    "from PyQt5.QtWebKitWidgets import QWebView as _W",
):
    try:
        _ns = {}
        exec(_imp, _ns)
        ...
        continue
if QWebView is None:
    # Last-resort fallback so the dock widget still loads.
    from qgis.PyQt.QtWidgets import QTextBrowser as _QTextBrowserFallback
    from qgis.PyQt.QtCore import QUrl as _QUrlFallback

    class QWebView(_QTextBrowserFallback):  # type: ignore[no-redef]
        """Minimal stand-in when no QtWebEngine/WebKit binding is available."""
        ...
        def load(self, url):
            try:
                if hasattr(url, "toLocalFile") and url.toLocalFile():
                    with open(url.toLocalFile(), "r", encoding="utf-8") as fh:
                        self.setHtml(fh.read())
                else:
                    self.setSource(url if not isinstance(url, str) else _QUrlFallback(url))
            except Exception as _e:
                self.setHtml(f"<pre>Failed to load page: {_e}</pre>")
from qgis.PyQt.QtCore import Qt, QCoreApplication
from qgis._core import QgsVectorLayer, QgsRasterLayer, QgsProcessing

QCoreApplication.setAttribute(Qt.ApplicationAttribute.AA_ShareOpenGLContexts)
from qgis.PyQt.QtWidgets import QDialog, QFileDialog, QTextEdit, QApplication, QWidget, QSizeGrip, QMessageBox, \
    QInputDialog, QTextBrowser, QProgressDialog
from qgis.PyQt.QtCore import QThread, pyqtSignal, QUrl, QObject, pyqtSlot, QPropertyAnimation, QPoint, QRect, QSettings
from qgis.PyQt import QtWidgets, uic
from qgis.PyQt.QtWidgets import QDialog, QHBoxLayout
from qgis.PyQt.QtWidgets import QApplication, QVBoxLayout, QHBoxLayout, QGridLayout, QWidget, QTextEdit, QPushButton, \
    QLabel, QLineEdit, QMenu, QCompleter
from qgis.PyQt.QtCore import QThread, pyqtSignal
from qgis.PyQt.QtGui import QTextCursor, QTextCharFormat, QFont, QColor, QPainter, QBrush, QSyntaxHighlighter, \
    QDesktopServices, QTextOption, QAction
...
class SpatialAnalysisAgentDockWidget(QtWidgets.QDockWidget, FORM_CLASS):
    ...
    def __init__(self, parent=None):
        ...
        try:
            ...
            print(f"[theme] failed to apply unified theme: {_theme_err}")
        self.resize(400, 2)
        ...
        self.task_completer.setCaseSensitivity(Qt.CaseSensitivity.CaseInsensitive)
        ...
        self.data_path_completer.setCaseSensitivity(Qt.CaseSensitivity.CaseInsensitive)
        ...
        self.agent.graph_ready.connect(self._on_agent_graph_ready, Qt.ConnectionType.QueuedConnection)
        self.agent.code_ready.connect(self.update_code_editor, Qt.ConnectionType.QueuedConnection)
        self.agent.plan_ready.connect(self._on_agent_plan_ready, Qt.ConnectionType.QueuedConnection)
        self.agent.result_ready.connect(self._on_agent_result_ready, Qt.ConnectionType.QueuedConnection)
        self.agent.task_finished.connect(self._on_agent_task_finished, Qt.ConnectionType.QueuedConnection)
        self.agent.token_summary_ready.connect(
            lambda msg: self.update_chatgpt_ans_textBrowser(msg, is_user=False),
            Qt.ConnectionType.QueuedConnection
        )
        # Phase 5: 知识库更新信号连接
        self.agent.knowledge_update_requested.connect(self._on_knowledge_update_requested, Qt.ConnectionType.QueuedConnection)
    ...
    def toggle_openai_key_field(self, model_name):
        ...
        try:
            ...
            if provider_name == 'ollama':
                ...
                self.OpenAI_key_LineEdit.setPlaceholderText("请输入 API 密钥")
            self.OpenAI_key_LineEdit.setStyleSheet("")
    ...
    def show_model_info_dialog(self):
        ...
        try:
            ...
            if not shown_before:
                ...
                dont_show_btn = msg_box.addButton("Don't show again", QMessageBox.ButtonRole.RejectRole)
    ...
    def run_script(self):
        ...
        try:
            ...
            if provider_name == 'ollama':
                ...
                self.update_chatgpt_ans_textBrowser(f"请输入 API 密钥", is_user=False)
                return
        except ImportError:
    ...
    def _markdown_to_html(text):
        ...
        def _replace_code_block(m):
            ...
            code_blocks[key] = (
                f'<pre style="background:#F0EEE6; padding:8px; border-radius:6px; '
                f'border:1px solid #E5E4DD; '
                f'font-family:Consolas,monospace; font-size:12px; overflow-x:auto; '
                f'white-space:pre-wrap; word-wrap:break-word; color:#3D3929;">'
        ...
        def _replace_inline_code(m):
            code = m.group(1)
            code = code.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
            return (f'<code style="background:#F0EEE6; padding:1px 4px; border-radius:3px; '
                    f'color:#3D3929; '
        ...
        for line in lines:
            ...
            if heading_match:
                ...
                result_lines.append(
                    ...
                    f'margin:10px 0 4px 0; color:#3D3929;">{title_text}</p>'
                )
                continue
    ...
    def append_text_with_format(self, text, is_user=True):
        ...
        if is_processing_status and is_user is False:
            message_style = f"color: #D97757; font-style: italic; font-size: 95%;"
        elif is_user is False:
            message_style = f"color: #3D3929;"
        else:
            message_style = f"color: {color_message};"

        html = f'''
            <div style="text-align: left; padding: 10px; margin: 5px; border: 1px solid #E5E4DD; border-radius: 8px; background:#FAF9F5;">
    ...
    def _setup_action_buttons(self):
        ...
        # ---- 按钮样式(走统一 theme 调色板) ----
        try:
            from theme import button_qss as _btn_qss
            confirm_style = _btn_qss("success")
            modify_style  = _btn_qss("warning")
            cancel_style  = _btn_qss("danger")
            neutral_style = _btn_qss("primary")
        except Exception:
            confirm_style = modify_style = cancel_style = neutral_style = ""
    ...
    def _run_via_agent(self):
        ...
        try:
            ...
            if provider_name != 'ollama' and not self.OpenAI_key:
                self.update_chatgpt_ans_textBrowser("请输入 API 密钥", is_user=False)
                return
        except ImportError:
            if not self.OpenAI_key:
                self.update_chatgpt_ans_textBrowser("请输入 API 密钥", is_user=False)
